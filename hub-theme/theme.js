/* BAZINO HUB ARENA THEME v2.2.0 — SDK v2, ES5 only.
...
(function () {
        ...
        h('div', { className: 'hb-wrap hb-in' },
          ...
          h('div', { className: 'hb-hright' },
            ...
            u ? h('span', { style: { display: 'inline-flex', alignItems: 'center', gap: 10 } },
              ...
              h('a', { href: '/profile', className: 'hb-user', onClick: function (e) { e.preventDefault(); go(p, '/profile'); } },
  ...
  function GamesPage(p) {
    ...
    var pickSt = useState(false); var pickOpen = pickSt[0], setPickOpen = pickSt[1];
    var confSt = useState(null); var pending = confSt[0], setPending = confSt[1];
    var systems = liveSystems(p);
    var extraRate = Number((p.settings || {}).extra_controller_hourly) || 25;
    var audience = view === 'kids' ? 'kids' : (view === 'adults' ? 'adults' : null);
    var grid = filterAudience(systems, audience);
    ...
    function confirmReserve() {
      if (!pending) return;
      var r = holdBay(p, pending, hours, extras, reqGame);
      setPickOpen(false); setPending(null);
    ...
    for (gi = 0; gi < grid.length; gi++) {
      (function (sys, idx) {
        ...
        stationCards.push(h('button', { key: sys.id, type: 'button', className: 'hb-price hb-price--pick' + (sys.isReserved ? ' is-busy' : ''), style: { '--c': c }, disabled: !!sys.isReserved, onClick: function () { if (!sys.isReserved) setPending(sys); } },
          h('div', { className: 'hb-price-art', style: { backgroundImage: 'url(' + art + ')' } }),
          h('div', { className: 'hb-price-body' },
            h('h3', null, sysLabel(p, sys)),
            h('small', null, sys.sub || (sys.type || 'PC')),
            h('div', { className: 'hb-price-tag' }, h('b', null, String(sys.hourlyRate || 0) + ' TL'), h('span', null, ts(p, 'common.hour', '/ HOUR'))),
            h('span', { className: 'hb-price-btn' }, sys.isReserved ? ts(p, 'games.busy', 'IN USE') : ts(p, 'games.pick', 'SELECT'))
          )));
      })(grid[gi], gi);
    }
    var pickModal = !pickOpen ? null : h('div', { className: 'hb-modalbg', onClick: function () { setPickOpen(false); setPending(null); } },
      h('div', { className: 'hb-modal hb-modal--scroll', onClick: function (e) { e.stopPropagation(); } },
        h('button', { type: 'button', className: 'hb-x', onClick: function () { setPickOpen(false); setPending(null); } }, '✕'),
        h('h2', { className: 'hb-pick-title' }, ts(p, 'games.pickTitle', 'CHOOSE A STATION')),
        h('p', { className: 'hb-book-hint' }, ts(p, 'games.pickHint', 'Select a free bay. Duration: ') + hours + 'h' + (reqGame ? ' · ' + reqGame : '')),
        grid.length ? h('div', { className: 'hb-stations' }, stationCards) : h('p', { className: 'hb-book-hint' }, ts(p, 'games.noSys', 'No systems yet.')),
        pending ? h('div', { className: 'hb-confirm' },
          h('p', null, ts(p, 'games.confirm', 'Reserve this station?'), ' ', h('b', null, sysLabel(p, pending))),
          h('div', { className: 'hb-confirm-row' },
            h('button', { type: 'button', className: 'hb-cta hb-cta--line', onClick: function () { setPending(null); } }, ts(p, 'games.cancel', 'CANCEL')),
            h('button', { type: 'button', className: 'hb-cta', onClick: confirmReserve }, ts(p, 'games.confirmGo', 'CONFIRM RESERVE'))
          )
        ) : null
      )
    );
    var bookBox = h('div', { className: 'hb-book hb-box hb-book--compact', style: { '--c': '#33cfff' } },
      h('div', { className: 'hb-card-head' }, h('span', { style: { color: '#33cfff' } }, ico(p, 'cal', 20)), h('b', null, ts(p, 'games.bookTitle', 'BOOK A STATION'))),
      h('div', { className: 'hb-book-compact' },
        h('div', { className: 'hb-book-row' }, h('small', null, ts(p, 'games.duration', 'Duration')), h('div', { className: 'hb-hbtns' }, hourBtns)),
        h('div', { className: 'hb-book-row' },
          h('small', null, ts(p, 'games.extras', 'Extra controllers')),
          h('div', { className: 'hb-step' },
            h('button', { type: 'button', onClick: function () { if (extras > 0) setExtras(extras - 1); } }, '−'),
            h('b', null, String(extras)),
            h('button', { type: 'button', onClick: function () { if (extras < 4) setExtras(extras + 1); } }, '+')
          )
        ),
      ...
      h('button', { type: 'button', className: 'hb-cta hb-hold-cta', onClick: openPicker }, ts(p, 'games.hold', 'HOLD MY BAY'))
    );
    ...
    return h('div', null, body, (view === 'kids' || view === 'adults') ? bookBox : null, pickModal);
  }
  ...
  function ClubPage(p) {
    ...
    var kv = [[ts(p,'club.fn','First Name'), name.split(' ')[0] || name], [ts(p,'club.ln','Last Name'), name.split(' ')[1] || '—'], [ts(p,'club.un','Username'), u.username || name], [ts(p,'club.id','BZN Member ID'), '#BZN1024'], [ts(p,'club.dob','Date of Birth'), '12 May 1998'], [ts(p,'club.ph','Phone Number'), u.phone || '—']];
    var kvs = [];
        ...
        h('div', { className: 'hb-prof-left' },
          h('div', { className: 'hb-box', style: { '--c': '#33cfff' } },
            h('div', { className: 'hb-card-head' }, h('span', { style: { color: '#33cfff' } }, ico(p, 'user', 24)), h('b', null, ts(p,'club.info','PERSONAL INFORMATION')),
              h('button', { type: 'button', className: 'hb-cta hb-cta--line', style: { '--c': '#33cfff', marginInlineStart: 'auto', padding: '6px 12px' }, onClick: function () { go(p, '/profile'); } }, ts(p,'club.edit','EDIT PROFILE'))),
            h('div', { className: 'hb-prof-main' },
              ...
              h('div', null,
                ...
                h('button', { type: 'button', className: 'hb-cta hb-cta--line', style: { '--c': '#ff2ea6', marginTop: 10 }, onClick: function () { go(p, '/profile/security'); } }, ico(p, 'lock', 15), ts(p,'club.pass','CHANGE PASSWORD'), '→')
              )
            )
          ),
