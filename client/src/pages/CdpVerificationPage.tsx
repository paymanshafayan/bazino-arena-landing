...
=======
  ...
  const scanCdp = async () => {
    setConnecting(true);
    setStatusMsg(`Connecting to Chrome CDP via server relay (Port ${cdpPort})...`);
    setErrorMsg("");
    try {
      const res = await fetch(`/api/cdp/tabs?port=${encodeURIComponent(cdpPort)}`);
      ...
      const list: ChromeTab[] = Array.isArray(data.tabs) ? data.tabs : [];
      const pageTabs = list.filter((t) => t.type === "page" || !t.type);
      setTabs(pageTabs);
      setStatusMsg(`Connected successfully! Found ${pageTabs.length} active page tabs.`);
    } catch (err: any) {
      console.error(err);
      setErrorMsg(
...
=======
  ...
  const captureTabCdp = async (tab: ChromeTab, themeTargetName: string) => {
    ...
    try {
      const outFileName = `${themeTargetName}.png`;
      ...
      const data = await res.json();
      if (!res.ok || !data.success) {
        throw new Error(data.error || "CDP command execution failed");
      }
