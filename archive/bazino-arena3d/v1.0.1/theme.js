(function () {
  ...
  function Home(p) {
    ...
    return h('main', { className: 'a3-root', ref: root, dir: dir },
      ...
      h('div', { className: 'a3-version-badge' }, 'BAZINO ARENA 3D — PREVIEW v1.0.1'),
      h('section', { className: 'a3-section a3-location', 'data-reveal': '1' }, h('header', null, h('small', null, '09 / FINAL COORDINATES'), h('h2', null, ts('location'))), h('div', { className: 'a3-map' }, h('img', { src: (p.settings && (p.settings.map_image_url || p.settings.mapImageUrl || p.settings.location_image)) || (base + 'location-map.png'), alt: '' }), h('div', { className: 'a3-radar', 'aria-hidden': 'true' }, h('i'), h('b')), h('div', { className: 'a3-contact' }, h('p', null, h('small', null, l === 'fa' ? 'آدرس' : 'Address'), h('strong', null, (p.settings && (p.settings.club_address || p.settings.address)) || ts('empty'))), (p.settings && (p.settings.club_phone || p.settings.phone || p.settings.contact_phone)) ? h('p', null, h('small', null, l === 'fa' ? 'تلفن تماس' : 'Phone'), h('a', { href: 'tel:' + (p.settings.club_phone || p.settings.phone || p.settings.contact_phone) }, p.settings.club_phone || p.settings.phone || p.settings.contact_phone)) : null))));
  }
  function PortalHeader(p) {
    p = p || {}; var l = p.language || p.lang || 'fa'; var user = p.user || null; var langs = ['fa','en','tr','ru']; var li = Math.max(0, langs.indexOf(l)); var nextLang = langs[(li + 1) % langs.length];
    return h('header', { className: 'a3-site-header', dir: l === 'fa' ? 'rtl' : 'ltr' }, h('button', { className: 'a3-site-brand', onClick: function () { if (p.onNavigate) p.onNavigate('home'); } }, p.logoUrl ? h('img', { src: p.logoUrl, alt: '' }) : h('b', null, 'B'), h('span', null, 'BAZINO ', h('i', null, 'PRO'))), h('div', { className: 'a3-site-actions' }, user ? h('div', { className: 'a3-user' }, h('span', null, user.displayName || user.username), typeof user.points === 'number' ? h('small', null, user.points + ' PT') : null, h('button', { onClick: p.onLogout }, l === 'fa' ? 'خروج' : 'Logout')) : h('button', { className: 'a3-login', onClick: p.onLogin }, l === 'fa' ? 'ورود' : 'Login'), h('button', { className: 'a3-lang', onClick: function () { if (p.onLanguage) p.onLanguage(nextLang); } }, l.toUpperCase())));
  }
