...
>
> 🎨 **قانون الزام اسکیل‌های طراحی (MANDATORY DESIGN SKILLS RULE):**  
> در کلیه درخواست‌های طراحی، بازطراحی و ساخت قالب یا کامپوننت‌های رابط کاربری، استفاده از اسکیل‌های سه‌گانه نصب‌شده (**Anthropic `frontend-design` & `theme-factory`**، **Emil Kowalski `emil-design-eng` & `animate`** و **`ui-ux-pro-max`**) الزامی و قطعی است.
>
...
- **قانون ۹ — الزام قطعی استفاده از اسکیل‌های سه‌گانه طراحی (Anthropic, Emil Kowalski, UI/UX Pro Max):** در تمامی درخواست‌های طراحی قالب (Theme)، لندینگ، صفحات هاب، کامپوننت‌ها و بازطراحی UI/UX، ایجنت موظف است حتماً و بدون استثنا از اسکیل‌های نصب‌شده در مخزن (`skills/` و `.claude/skills/`) به شرح زیر استفاده نماید:
  ۱) **اسکیل‌های رسمی انتروپیک (Anthropic Skills - به‌ویژه `frontend-design` و `theme-factory`):** هدایت طراحی با رویکرد سرپرست طراحی (Design Lead)، هویت بصری ارگانیک و اختصاصی بدون استفاده از الگوهای کلیشه‌ای و خسته‌کننده هوش مصنوعی، انتخاب دقیق تایپوگرافی، پالت رنگی ۴ تا ۶ تایی هماهنگ با موضوع گیم‌نت و بازینو، و پرهیز از تله‌های طراحی جنریک.
  ۲) **اسکیل‌های مهندسی طراحی و انیمیشن امیل کوالسکی (Emil Kowalski Skills - به‌ویژه `emil-design-eng` و `animate`):** تنظیم دقیق فیزیک تعاملات و اسپرینگ، دکمه‌های با بازخورد لمسی سریع (`:active` فیدبک آنی)، بررسی تغییرات با جدول استاندارد `Before | After | Why`، رعایت `prefers-reduced-motion` و ممانعت از انیمیشن‌های غیرضروری یا کندکننده.
  ۳) **موتور هوش طراحی UI/UX Pro Max (`ui-ux-pro-max`):** استفاده از دیتاست‌های محلی، جستجوگر CLI (`skills/ui-ux-pro-max/scripts/search.py`)، استخراج توکن‌های دارک‌مود و نئون، رعایت کنتراست WCAG 4.5:1، رعایت ابعاد تاچ ۴۴×۴۴ پیکسل و جلوگیری از شیفت چیدمان (CLS < 0.1).
