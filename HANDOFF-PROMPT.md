...
> **به هیچ عنوان تا زمانی که قالب جدید را بر روی سرور زنده پورتال بازینو داخل سندباکس نصب نکرده، فعال‌سازی کامل را تست نکرده، چک بصری دقیق انجام نداده و اسکرین‌شات زنده از محیط واقعی پورتال نگرفته‌اید، فایل زیپ (ZIP) برای نصب به کاربر تحویل نمی‌دهید.**  
> هرگونه تحویل بسته زیپ بدون تست و نصب واقعی روی سرور زنده پورتال در سندباکس و اخذ اسکرین‌شات چشمی اکیداً ممنوع است.

> **به‌روزرسانی 2026-09-09 (تست کامل روی سرور واقعی پورتال بازینو در سندباکس + فیکس کارخانه SDK v2 + تایید با اسکرین‌شات):**
> (۱) سرور واقعی پورتال بازینو (`paymanshafayan/bazino-gamenet-portal`) در سندباکس بالا آورده شد، متغیرهای محیطی و دیتابیس لوکال ست شدند و هر دو بسته `bazino-arena-theme.zip` (v4.5.5) و `bazino-hub-theme.zip` (v1.0.0) از طریق اندپوینت `/api/admin/themes/install` نصب و با `/api/admin/themes/activate` تست و فعال شدند؛
> (۲) باگ اعتبارسنجی CSS پورتال (`body[data-theme='...']` و `.theme-...`) برطرف شد؛
> (۳) تابع کارخانه SDK v2 به الگوی دوگانه `makeFactory` اصلاح شد تا خطای React Object Child رخ ندهد؛
> (۴) اسکرین‌شات‌های کیفیت بالای هر دو قالب روی پورتال در فایل‌های `portal-arena-theme-screenshot.png` و `portal-hub-theme-screenshot.png` ثبت و کامیت شدند.
