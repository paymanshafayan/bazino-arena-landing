...
## ۱۴) وضعیت تأییدشدهٔ Hero جدید و شروع فاز Landing Page (2026-09-12)

### ۱۴-۱) خروجی فعلی Hero

- قالب قدیمی صفحهٔ اصلی کنار گذاشته شد و مسیر `/` مستقیماً Hero جدید را نمایش می‌دهد؛ مسیرهای `/3d` و `/dimension` نیز برای بررسی همین Hero باقی مانده‌اند.
- منبع Motion، ویدئوی دقیق `SnapInsta.to_AQP111EBUemx4CPUXxSNiPhrITNbV7BQwayq8mCz1FucR1t89W3OgiPoHu6cYTSB0GKZT_TWtEQ91D31IIQKsKIcrRLEp30p_lAuisw.mp4` با مدت اندازه‌گیری‌شدهٔ ۳۳٫۲۳ ثانیه است.
- ویدئو با نرخ ۱۲fps نمونه‌برداری شد: ۳۹۹ فریم خام استخراج و پس از حذف فریم‌های تکراری/بسیار مشابه، ۳۹۴ فریم WebP در `client/public/assets/video-sequence/` ذخیره شد. جزئیات در `manifest.json` همان پوشه ثبت شده است.
- Hero هیچ `<video>` خامی ندارد؛ sequence توسط Canvas و `requestAnimationFrame`، متناسب با حرکت افقی ماوس scrub می‌شود.
- Hero بدون intro، click gate، capsule یا راهنمای Mouse/Drag مستقیماً از فریم اول آغاز می‌شود.
- روایت UI روی sequence: ورود/راهرو → شخصیت کلاه‌دار → FC/PS5 → CTA رزرو.
- فونت فارسی Hero نسخه‌های محلی Vazirmatn در `theme-package/assets/` است؛ هیچ درخواست خارجی برای فونت یا asset ایجاد نمی‌شود.
- متن و CTA دارای reveal، glow، gradient و hover/focus هستند.
- نشانگر سفارشی و هالهٔ آبی شفاف فقط داخل Hero فعال است؛ روی موبایل/ورودی coarse غیرفعال می‌شود و روی بخش Landing نشانگر عادی باقی می‌ماند.
- کارفرما sequence جدید را با عبارت «خیلی خوب شد» تأیید کرد و سپس درخواست commit/push و حرکت به فاز Landing Page را داد.

### ۱۴-۲) وضعیت Landing Page و گام بعدی

- بخش زیر Hero عمداً یک صفحهٔ خالی مشکی با ارتفاع یک viewport است؛ هنوز هیچ طراحی Landing Page در آن انجام نشده است.
- گام بعدی فقط طراحی Landing Page است. Hero تأییدشده، sequence، رفتار scrub و هالهٔ محدود به Hero نباید بدون درخواست صریح کارفرما بازطراحی یا با روش دیگری جایگزین شوند.
- Chat همچنان برای همیشه حذف است.
- بستهٔ نهایی Theme/ZIP هنوز ساخته نشده و باید پس از تکمیل و تأیید Landing Page انجام شود.

### ۱۴-۳) شواهد فنی آخرین اعتبارسنجی

- `corepack pnpm run check` موفق شد.
- `corepack pnpm run build` موفق شد.
- هشدارهای موجود build مربوط به متغیرهای تعریف‌نشدهٔ Analytics در `index.html` و اندازهٔ chunk هستند؛ build را fail نکردند.
