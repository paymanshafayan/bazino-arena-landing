(function () {
  ...
  function Home(p) {
    ...
    return h('main', { className: 'a3-root', ref: root, dir: dir },
      ...
      h('div', { className: 'a3-version-badge' }, 'BAZINO ARENA 3D — PREVIEW v1.1.1'),
  ...
  function innerList(p, region) {
    if (region.indexOf('games') === 0) return p.games || p.featuredGames || [];
  ...
  function invoke(p, region, x) {
    ...
    if (region === 'games' || region === 'games.detail') { if (p.onViewDetail) p.onViewDetail(id); return; }
    if (region === 'blog' && p.onOpenArticle) { p.onOpenArticle(id); return; }
    if (region.indexOf('tournaments') === 0) { if (p.onOpenBracket) { p.onOpenBracket(id); return; } if (p.onNavigate) { p.onNavigate('/tournaments/brackets'); return; } }
  }
  ...
  function InnerPage(p) {
    ...
    var cartState = R.useState(p.cart || []), cart = cartState[0], setCart = cartState[1];
    var focusState = R.useState(null), focus = focusState[0], setFocus = focusState[1];
    var teamState = R.useState(''), teamName = teamState[0], setTeamName = teamState[1];
    var leaderState = R.useState(''), leader = leaderState[0], setLeader = leaderState[1];
    function addItem(x) { var found = false, next = cart.map(function (row) { var item = row.item || row; if (item.id === x.id) { found = true; return { item: item, qty: (row.qty || 1) + 1 }; } return row; }); if (!found) next.push({ item: x, qty: 1 }); setCart(next); if (p.addNotification) p.addNotification((itemName(x,l) || 'Item') + (l === 'fa' ? ' اضافه شد' : ' added'), 'success'); }
    function removeItem(id) { setCart(cart.filter(function (row) { return (row.item || row).id !== id; })); }
    function checkout(kind) { var total = cart.reduce(function (n,row) { var item=row.item||row; return n + Number(item.price || 0) * (row.qty || 1); }, 0); if (p.onCheckout) p.onCheckout(kind, { items: cart.map(function(row){return { id:(row.item||row).id, qty:row.qty||1 };}) }, total); }
    function cardAction(x) { if (region === 'cafe' || region === 'cafe.detail' || region === 'shop' || region === 'shop.detail') return addItem(x); if (region === 'blog') return setFocus(x); if (region === 'games') return setFocus(x); return invoke(p,region,x); }
    ...
    return h('main', { className: 'a3i-root a3i-' + c[4], dir: p.dir || (l === 'fa' ? 'rtl' : 'ltr') },
      ...
      region === 'games' ? h('section', { className: 'a3i-game-gates' },
        [['kids','کودکان','KIDS'],['adults','بزرگسالان','ADULTS'],['requests','درخواست بازی','GAME REQUEST']].map(function(g){return h('button',{key:g[0],onClick:function(){setFocus({audience:g[0]});}},l==='fa'?g[1]:g[2]);}),
        focus ? h('div',{className:'a3i-reserve'},h('strong',null,l==='fa'?'رزرو ایستگاه گیمینگ':'BOOK A GAMING STATION'),h('p',null,l==='fa'?'سیستم مورد نظر را از کارت‌های زیر انتخاب کنید. پرداخت توسط درگاه واقعی پورتال انجام می‌شود.':'Choose a system below. Checkout uses the portal payment flow.'),h('button',{onClick:function(){if(p.onCheckout){var system=list[0];p.onCheckout('reservation',{systemId:system&&system.id,audience:focus.audience,requestedGame:focus.id||''},Number(system&&system.price||0));}}},l==='fa'?'ادامه رزرو':'CONTINUE BOOKING')):null) : null,
      focus && region === 'blog' ? h('section',{className:'a3i-article-detail'},h('button',{onClick:function(){setFocus(null);}},l==='fa'?'بازگشت':'BACK'),h('h2',null,itemName(focus,l)),h('p',null,txt(focus.content||focus.description,l)),h('div',null,(focus.comments||[]).map(function(cm,i){return h('blockquote',{key:i},txt(cm.content||cm.text,l));}))) : null,
      (region === 'cafe' || region === 'shop') && cart.length ? h('aside',{className:'a3i-live-cart'},h('strong',null,l==='fa'?'سبد خرید':'CART'),cart.map(function(row){var item=row.item||row;return h('div',{key:item.id},h('span',null,itemName(item,l)+' × '+(row.qty||1)),h('button',{onClick:function(){removeItem(item.id);}},'×'));}),h('button',{onClick:function(){checkout(region);}},l==='fa'?'ادامه پرداخت':'CHECKOUT')) : null,
      region === 'tournaments' ? h('section',{className:'a3i-event-console'},
        [['/tournaments/weekly','WEEKLY','مسابقات هفتگی'],['/tournaments/special','SPECIAL','رویدادهای ویژه'],['/tournaments/season','SEASON','فصل مسابقات'],['/tournaments/brackets','BRACKETS','جدول مسابقات'],['/tournaments/register','REGISTER','ثبت‌نام تیم']].map(function(tab){return h('button',{key:tab[0],onClick:function(){if(p.onNavigate)p.onNavigate(tab[0]);}},h('small',null,tab[1]),h('strong',null,l==='fa'?tab[2]:tab[1]));})) : null,
      region === 'tournaments.brackets' ? h('section',{className:'a3i-bracket-board'},
        ['round1','semis','finals'].map(function(round){var source=(p.bracket&&p.bracket[round])||(p.selectedTournament&&p.selectedTournament.bracket&&p.selectedTournament.bracket[round])||[];return h('div',{key:round},h('h3',null,round.toUpperCase()),source.length?source.map(function(match,i){return h('article',{key:i},h('span',null,itemName(match.teamA||match.player1||{},l)||txt(match.teamA||match.player1,l)||'—'),h('b',null,String(match.scoreA!=null?match.scoreA:'–')+' : '+String(match.scoreB!=null?match.scoreB:'–')),h('span',null,itemName(match.teamB||match.player2||{},l)||txt(match.teamB||match.player2,l)||'—'));}):h('p',null,l==='fa'?'داده جدول هنوز از پورتال دریافت نشده است.':'Bracket data has not arrived from the portal.'));})) : null,
      region === 'tournaments.register' ? h('form',{className:'a3i-register-form',onSubmit:function(e){e.preventDefault();var tournament=list[0]||p.selectedTournament;if(p.onRegisterTeam&&tournament)p.onRegisterTeam(tournament.id,{name:teamName,leader:leader,members:[]});}},h('small',null,'REAL PORTAL REGISTRATION'),h('label',null,l==='fa'?'نام تیم':'TEAM NAME',h('input',{value:teamName,required:true,onChange:function(e){setTeamName(e.target.value);}})),h('label',null,l==='fa'?'نام سرگروه':'TEAM LEADER',h('input',{value:leader,required:true,onChange:function(e){setLeader(e.target.value);}})),h('button',{type:'submit',disabled:!list.length},l==='fa'?'ثبت تیم':'REGISTER TEAM')) : null,
      (region !== 'tournaments.brackets' && region !== 'tournaments.register') && list.length ? h('section', { className: 'a3i-cards' }, list.map(function (x, i) { return h('article', { className: 'a3i-card', key: (x && (x.id || x.slug)) || i }, h('div', { className: 'a3i-media' }, h('img', { src: itemImage(x) || (base + 'portal-menu/' + c[3]), alt: '' }), h('span', null, pad(i + 1))), h('div', { className: 'a3i-copy' }, h('small', null, c[0]), h('h2', null, itemName(x, l) || title), itemDesc(x, l) ? h('p', null, itemDesc(x, l)) : null, h('button', { onClick: function () { cardAction(x); } }, (l === 'fa' ? ((region.indexOf('cafe')===0||region.indexOf('shop')===0) ? 'افزودن به سبد' : 'مشاهده') : ((region.indexOf('cafe')===0||region.indexOf('shop')===0) ? 'ADD TO CART' : 'OPEN')), h('i', null, ' ↗')))); })) : (c[4] !== 'contact' && c[4] !== 'loyalty' ? h(EmptySignal, p) : null),
  ...
  function PortalHeader(p) {
    ...
    return h('header', { className: 'a3-site-header', dir: l === 'fa' ? 'rtl' : 'ltr' },
      h('button', { className: 'a3-site-brand', 'aria-label': 'Home', onClick: function () { if (p.onNavigate) p.onNavigate('/'); } }, p.logoUrl ? h('img', { src: p.logoUrl, alt: '' }) : h('b', null, 'B'), h('span', null, 'BAZINO ', h('i', null, 'PRO'))),
      h('nav', { className: 'a3-main-nav', 'aria-label': 'Main navigation' }, dockItems.map(function (d) { return h('button', { key: d[0], className: p.activeTab === d[0] || (d[0] === 'tournaments' && p.activeTab === 'events') ? 'is-active' : '', onClick: function () { if (p.onNavigate) p.onNavigate(d[5]); } }, l === 'fa' ? d[1] : d[2]); })),
